...
def _copy_webkit_files() -> None:
    ...
    logger.log(f"Copying GameGen web UI from {js_src} to {js_dst}")
    try:
        shutil.copy(js_src, js_dst)
    except Exception as exc:
        logger.error(f"Failed to copy GameGen web UI: {exc}")
    ...
    if os.path.exists(icon_src):
        try:
            ...
            logger.log(f"Copied GameGen icon to {icon_dst}")
        except Exception as exc:
            logger.error(f"Failed to copy GameGen icon: {exc}")
    else:
        logger.warn(f"GameGen icon not found at {icon_src}")
...
def _inject_webkit_files() -> None:
    ...
    logger.log(f"GameGen injected web UI: {js_path}")
...
def HasGameGenForApp(appid: int, contentScriptQuery: str = "") -> str:
    return has_GameGen_for_app(appid)


def StartAddViaGameGen(appid: int, contentScriptQuery: str = "") -> str:
    return start_add_via_GameGen(appid)


def GetAddViaGameGenStatus(appid: int, contentScriptQuery: str = "") -> str:
...
def CancelAddViaGameGen(appid: int, contentScriptQuery: str = "") -> str:
    return cancel_add_via_GameGen(appid)
...
def GetMorrenusStats(api_key: str, force_refresh: bool = False, contentScriptQuery: str = "", **kwargs: Any) -> str:
    ...
    try:
        from http_client import ensure_http_client
        client = ensure_http_client("GameGen: GetMorrenusStats")
        ...
        logger.warn(f"GameGen: GetMorrenusStats failed: {exc}")
        return json.dumps({"error": str(exc)})


def StartAddViaGameGenFromUrl(appid: int, url: str, apiName: str, contentScriptQuery: str = "") -> str:
    return start_add_via_GameGen_from_url(appid, url, apiName)
...
def DeleteGameGenForApp(appid: int, contentScriptQuery: str = "") -> str:
    return delete_GameGen_for_app(appid)
...
def OpenExternalUrl(url: str, contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"GameGen: OpenExternalUrl failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetSettingsConfig(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"GameGen: GetSettingsConfig failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def ApplySettingsChanges(
    ...
    try:
        ...
        try:
            logger.log(
                "GameGen: ApplySettingsChanges raw argument "
                f"type={type(changes)} value={changes!r}"
            )
            logger.log(f"GameGen: ApplySettingsChanges kwargs: {kwargs}")
        except Exception:
            pass
        ...
        if isinstance(changes, str) and changes:
            try:
                ...
                logger.warn("GameGen: Failed to parse changes string payload")
                ...
                if isinstance(payload, dict) and "changes" in payload:
                    ...
                    try:
                        ...
                        logger.warn("GameGen: Failed to parse changesJson string inside payload")
                        return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if "changesJson" in changes and isinstance(changes["changesJson"], str):
                try:
                    ...
                    logger.warn("GameGen: Failed to parse changesJson payload from dict")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if isinstance(changes_json, dict):
                ...
                try:
                    ...
                    logger.warn("GameGen: Failed to parse changesJson payload")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            else:
                payload = changes
        ...
        if payload is None:
            ...
            logger.warn(f"GameGen: Parsed payload is not a dict: {payload!r}")
            return json.dumps({"success": False, "error": "Invalid payload format"})

        try:
            logger.log(f"GameGen: ApplySettingsChanges received payload: {payload}")
        except Exception:
            pass

        result = apply_settings_changes(payload)
        try:
            logger.log(f"GameGen: ApplySettingsChanges result: {result}")
        except Exception:
            pass
        response = json.dumps(result)
        try:
            logger.log(f"GameGen: ApplySettingsChanges response json: {response}")
        except Exception:
            pass
        return response
    except Exception as exc:
        logger.warn(f"GameGen: ApplySettingsChanges failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableLocales(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"GameGen: GetAvailableLocales failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetTranslations(contentScriptQuery: str = "", language: str = "", **kwargs: Any) -> str:
    try:
        ...
        logger.warn(f"GameGen: GetTranslations failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableThemes(contentScriptQuery: str = "") -> str:
    ...
    try:
        ...
        logger.warn(f"GameGen: GetAvailableThemes failed: {exc}")
...
class Plugin:
    ...
    def _load(self):
        logger.log(f"bootstrapping GameGen plugin, millennium {Millennium.version()}")

        try:
            detect_steam_install_path()
        except Exception as exc:
            logger.warn(f"GameGen: steam path detection failed: {exc}")
        ...
        try:
            ...
            logger.warn(f"GameGen: settings initialization failed: {exc}")
        ...
        try:
            ...
            logger.warn(f"GameGen: Fixes index initialization failed: {exc}")
