"""Handling of GameGen add/download flows and related utilities."""
...
def _fetch_app_name(appid: int) -> str:
    ...
    client = ensure_http_client("GameGen: _fetch_app_name")
    try:
        url = f"https://store.steampowered.com/api/appdetails?appids={appid}"
        logger.log(f"GameGen: Fetching app name for {appid} from Steam API")
        resp = client.get(url, follow_redirects=True, timeout=10)
        logger.log(f"GameGen: Steam API response for {appid}: status={resp.status_code}")
        ...
        logger.warn(f"GameGen: _fetch_app_name failed for {appid}: {exc}")
...
def _fetch_app_info(appid: int) -> dict:
    ...
    client = ensure_http_client("GameGen: _fetch_app_info")
    try:
        url = f"https://api.steamcmd.net/v1/info/{appid}"
        logger.log(f"GameGen: Fetching app info for {appid} from steamcmd.net")
        resp = client.get(url, follow_redirects=True, timeout=10)
        logger.log(f"GameGen: steamcmd.net response for {appid}: status={resp.status_code}")
        resp.raise_for_status()
        ...
        logger.warn(f"GameGen: _fetch_app_info failed for {appid}: {exc}")
...
def _append_loaded_app(appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"GameGen: _append_loaded_app failed for {appid}: {exc}")
...
def _remove_loaded_app(appid: int) -> None:
    try:
        ...
        logger.warn(f"GameGen: _remove_loaded_app failed for {appid}: {exc}")
...
def _log_appid_event(action: str, appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"GameGen: _log_appid_event failed: {exc}")
...
def _preload_app_names_cache() -> None:
    ...
    try:
        ...
        logger.warn(f"GameGen: _preload_app_names_cache from logs failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"GameGen: _preload_app_names_cache from loaded_apps failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"GameGen: _preload_app_names_cache from applist failed: {exc}")
...
def _load_applist_into_memory() -> None:
    ...
    with APPLIST_LOCK:
        ...
        if not os.path.exists(file_path):
            logger.log("GameGen: Applist file not found, skipping load")
            APPLIST_LOADED = True  # Mark as loaded to avoid repeated checks
            return
        
        try:
            logger.log("GameGen: Loading applist into memory...")
                ...
                for entry in data:
                    if isinstance(entry, dict):
                        ...
                        if appid and name and isinstance(name, str) and name.strip():
                            ...
                            count += 1
                logger.log(f"GameGen: Loaded {count} app names from applist into memory")
            else:
                logger.warn("GameGen: Applist file has invalid format (expected array)")
            
            APPLIST_LOADED = True
        except Exception as exc:
            logger.warn(f"GameGen: Failed to load applist into memory: {exc}")
...
def _ensure_applist_file() -> None:
    ...
    if os.path.exists(file_path):
        logger.log("GameGen: Applist file already exists, skipping download")
        return
    
    logger.log("GameGen: Applist file not found, downloading...")
    client = ensure_http_client("GameGen: DownloadApplist")
    
    try:
        logger.log(f"GameGen: Downloading applist from {APPLIST_URL}")
        resp = client.get(APPLIST_URL, follow_redirects=True, timeout=APPLIST_DOWNLOAD_TIMEOUT)
        logger.log(f"GameGen: Applist download response: status={resp.status_code}")
        resp.raise_for_status()
        ...
        try:
            ...
            if not isinstance(data, list):
                logger.warn("GameGen: Downloaded applist has invalid format (expected array)")
                return
        except json.JSONDecodeError as exc:
            logger.warn(f"GameGen: Downloaded applist is not valid JSON: {exc}")
            return
        ...
        logger.log(f"GameGen: Successfully downloaded and saved applist file ({len(data)} entries)")
    except Exception as exc:
        logger.warn(f"GameGen: Failed to download applist file: {exc}")
...
def init_applist() -> None:
    ...
    try:
        ...
        logger.warn(f"GameGen: Applist initialization failed: {exc}")
...
def _load_games_db_into_memory() -> None:
    ...
    with GAMES_DB_LOCK:
        ...
        if not os.path.exists(file_path):
            logger.log("GameGen: Games DB file not found, skipping load")
            GAMES_DB_LOADED = True
            return
        
        try:
            logger.log("GameGen: Loading Games DB into memory...")
            with open(file_path, "r", encoding="utf-8") as handle:
                GAMES_DB_DATA = json.load(handle)
            
            logger.log(f"GameGen: Loaded Games DB ({len(GAMES_DB_DATA)} entries)")
            GAMES_DB_LOADED = True
        except Exception as exc:
            logger.warn(f"GameGen: Failed to load Games DB: {exc}")
            GAMES_DB_LOADED = True
...
def _ensure_games_db_file() -> None:
    ...
    if os.path.exists(file_path) and not _is_games_db_cache_stale():
        logger.log("GameGen: Games DB cache is fresh, skipping download")
        return

    logger.log("GameGen: Downloading Games DB (cache missing or stale)...")
    client = ensure_http_client("GameGen: DownloadGamesDB")
    
    try:
        logger.log(f"GameGen: Downloading Games DB from {GAMES_DB_URL}")
        resp = client.get(GAMES_DB_URL, follow_redirects=True, timeout=60)
        logger.log(f"GameGen: Games DB download response: status={resp.status_code}")
        resp.raise_for_status()
        ...
        logger.log(f"GameGen: Successfully downloaded Games DB")
    except Exception as exc:
        logger.warn(f"GameGen: Failed to download Games DB: {exc}")
...
def init_games_db() -> None:
    ...
    try:
        ...
        logger.warn(f"GameGen: Games DB initialization failed: {exc}")
...
def _process_and_install_lua(appid: int, zip_path: str) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as archive:
            ...
            for name in names:
                try:
                    ...
                    if name.lower().endswith(".manifest"):
                        ...
                        logger.log(f"GameGen: Extracted manifest -> {out_path}")
                except Exception as manifest_exc:
                    logger.warn(f"GameGen: Failed to extract manifest {name}: {manifest_exc}")
        except Exception as depot_exc:
            logger.warn(f"GameGen: depotcache extraction failed: {depot_exc}")
        ...
        logger.log(f"GameGen: Installed lua -> {dest_file}")
        ...
        try:
            ...
            logger.error(f"GameGen: Error checking lua for app {appid}: {exc}")
...
def _download_zip_for_app(appid: int):
    client = ensure_http_client("GameGen: download")
    apis = load_api_manifest()
    if not apis:
        logger.warn("GameGen: No enabled APIs in manifest")
    ...
    for api in apis:
        ...
        if "<moapikey>" in template:
            if not morrenus_api_key:
                ...
                logger.log(f"GameGen: Skipping API '{name}' - Morrenus API key not configured")
                continue
        ...
        logger.log(f"GameGen: Trying API '{name}'")
        try:
            headers = {"User-Agent": USER_AGENT}
            if _is_download_cancelled(appid):
                logger.log(f"GameGen: Download cancelled before contacting API '{name}'")
                return
            with client.stream("GET", url, headers=headers, follow_redirects=True) as resp:
                code = resp.status_code
                logger.log(f"GameGen: API '{name}' status={code}")
                ...
                with open(dest_path, "wb") as output:
                    for chunk in resp.iter_bytes():
                        if not chunk:
                            continue
                        if _is_download_cancelled(appid):
                            logger.log(f"GameGen: Download cancelled mid-stream for appid={appid}")
                            raise RuntimeError("cancelled")
                        output.write(chunk)
                        state = _get_download_state(appid)
                        ...
                        if _is_download_cancelled(appid):
                            logger.log(f"GameGen: Download cancelled after writing chunk for appid={appid}")
                            raise RuntimeError("cancelled")
                logger.log(f"GameGen: Download complete -> {dest_path}")

                if _is_download_cancelled(appid):
                    logger.log(f"GameGen: Download marked cancelled after completion for appid={appid}")
                    raise RuntimeError("cancelled")
                ...
                try:
                    with open(dest_path, "rb") as fh:
                        ...
                        if magic not in (b"PK\x03\x04", b"PK\x05\x06", b"PK\x07\x08"):
                            ...
                            logger.warn(
                                f"GameGen: API '{name}' returned non-zip file (magic={magic.hex()}, size={file_size}, preview={content_preview[:50]})"
                            )
                            try:
                                os.remove(dest_path)
                            except Exception:
                    ...
                    logger.warn("GameGen: Downloaded file not found after download")
                    continue
                except Exception as validation_exc:
                    logger.warn(f"GameGen: File validation failed for API '{name}': {validation_exc}")
                    try:
                        os.remove(dest_path)
                    except Exception:
                        pass
                ...
                try:
                    if _is_download_cancelled(appid):
                        logger.log(f"GameGen: Processing aborted due to cancellation for appid={appid}")
                        raise RuntimeError("cancelled")
                    _set_download_state(appid, {"status": "processing"})
                    _process_and_install_lua(appid, dest_path)
                    if _is_download_cancelled(appid):
                        logger.log(f"GameGen: Installation complete but marked cancelled for appid={appid}")
                        raise RuntimeError("cancelled")
                    try:
                    ...
                    if isinstance(install_exc, RuntimeError) and str(install_exc) == "cancelled":
                        try:
                            ...
                            pass
                        logger.log(f"GameGen: Cancelled download cleanup complete for appid={appid}")
                        return
                    logger.warn(f"GameGen: Processing failed -> {install_exc}")
                    _set_download_state(
            ...
            if str(cancel_exc) == "cancelled":
                try:
                    ...
                    pass
                logger.log(f"GameGen: Download cancelled and cleaned up for appid={appid}")
                return
            logger.warn(f"GameGen: Runtime error during download for appid={appid}: {cancel_exc}")
            _set_download_state(appid, {"status": "failed", "error": str(cancel_exc)})
            return
        except Exception as err:
            logger.warn(f"GameGen: API '{name}' failed with error: {err}")
...
def check_apis_for_app(appid: int) -> str:
    ...
    client = ensure_http_client("GameGen: check_apis")
    apis = load_api_manifest()
    if not apis:
    ...
    try:
        ...
        logger.warn(f"GameGen: Fast API check failed: {exc}")
...
def _download_zip_from_url(appid: int, url: str, api_name: str):
    ...
    client = ensure_http_client("GameGen: download_direct")
    dest_root = ensure_temp_download_dir()
    dest_path = os.path.join(dest_root, f"{appid}.zip")
    ...
    try:
        ...
        if str(exc) == "cancelled":
            ...
            logger.warn(f"GameGen: _download_zip_from_url failed: {exc}")
            _set_download_state(appid, {"status": "failed", "error": str(exc)})


def start_add_via_GameGen_from_url(appid: int, url: str, api_name: str) -> str:
...
def start_add_via_GameGen(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})

    logger.log(f"GameGen: StartAddViaGameGen appid={appid}")
...
def delete_GameGen_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    for path in paths:
        try:
            ...
            logger.warn(f"GameGen: Failed to delete {path}: {exc}")
    try:
...
def get_icon_data_url() -> str:
    try:
        ...
        logger.warn(f"GameGen: GetIconDataUrl failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


def has_GameGen_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
...
def cancel_add_via_GameGen(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    _set_download_state(appid, {"status": "cancelled", "error": "Cancelled by user"})
    logger.log(f"GameGen: Cancellation requested for appid={appid}")
    return json.dumps({"success": True})
...
def get_installed_lua_scripts() -> str:
    ...
    try:
            ...
            for filename in os.listdir(target_dir):
                ...
                if filename.endswith(".lua") or filename.endswith(".lua.disabled"):
                    try:
                        ...
                        logger.warn(f"GameGen: Failed to process Lua file {filename}: {exc}")
                        continue

        except Exception as exc:
            logger.warn(f"GameGen: Failed to scan stplug-in directory: {exc}")
        ...
        logger.warn(f"GameGen: Failed to get installed Lua scripts: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


__all__ = [
    "cancel_add_via_GameGen",
    "delete_GameGen_for_app",
    "dismiss_loaded_apps",
    "fetch_app_name",
    "fetch_app_info",
    "get_add_status",
    ...
    "init_games_db",
    "read_loaded_apps",
    "start_add_via_GameGen",
    "check_apis_for_app",
    "start_add_via_GameGen_from_url",
]
